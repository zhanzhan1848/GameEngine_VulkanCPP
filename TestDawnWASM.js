...
var ASM_CONSTS = {
 ...
 7851394: ($0) => { if (window.setSunAutoRotate) window.setSunAutoRotate($0); },  
 7851456: ($0, $1, $2) => { if (window.setRenderMode) { window.setRenderMode($0, UTF8ToString($1), UTF8ToString($2)); } },  
 7851552: ($0) => { if (window.setMeshletDebug) { window.setMeshletDebug($0); } },  
 7851616: ($0) => { if (window.setSSGISSRSubmode) { window.setSSGISSRSubmode($0); } },  
 7851684: () => { try { FS.mkdir('/persist'); } catch (e) { } try { FS.mount(IDBFS, {}, '/persist'); } catch (e) { } window._ddgiSyncDone = false; FS.syncfs(true, function(err) { window._ddgiSyncDone = true; if (err) console.warn('[Prebake] IDBFS syncfs(true) error:', err); }); },  
 7851949: () => { return window._ddgiSyncDone ? 1 : 0; },  
 7851990: () => { try { FS.stat('/persist/mode10_ddgi_cache.spch'); return 1; } catch (e) { return 0; } },  
 7852080: () => { var data = FS.readFile('/persist/mode10_ddgi_cache.spch'); FS.writeFile('mode10_ddgi_cache.spch', data); },  
 7852189: () => { try { FS.stat('mode10_ddgi_cache.spch'); return 1; } catch (e) { return 0; } },  
 7852270: () => { try { var data = FS.readFile('mode10_ddgi_cache.spch'); FS.writeFile('/persist/mode10_ddgi_cache.spch', data); window._ddgiSyncDone = false; FS.syncfs(false, function(err) { window._ddgiSyncDone = true; if (err) console.warn('[Prebake] IDBFS syncfs(false) error:', err); }); } catch (e) { console.error('[Prebake] Failed to persist cache:', e); window._ddgiSyncDone = true; } },  
 7852650: () => { return window._ddgiSyncDone ? 1 : 0; },  
 7852691: () => { var label = document.querySelector('#loading .label'); if (label) label.textContent = 'Loading...'; }
};
