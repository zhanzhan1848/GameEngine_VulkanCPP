...
var ASM_CONSTS = {
 ...
 7850050: ($0) => { if (window.setSunAutoRotate) window.setSunAutoRotate($0); },  
 7850112: ($0, $1, $2) => { if (window.setRenderMode) { window.setRenderMode($0, UTF8ToString($1), UTF8ToString($2)); } },  
 7850208: ($0) => { if (window.setMeshletDebug) { window.setMeshletDebug($0); } },  
 7850272: ($0) => { if (window.setSSGISSRSubmode) { window.setSSGISSRSubmode($0); } },  
 7850340: () => { var label = document.querySelector('#loading .label'); if (label) label.textContent = 'Loading...'; },  
 7850444: ($0, $1, $2) => { var label = document.querySelector('#loading .label'); if (label) { label.textContent = 'Baking DDGI probes (' + UTF8ToString($0) + '): ' + $1 + ' / ' + $2; } }
};
