...
namespace primal::tools
{
	namespace
	{
		...
		mesh create_plane(const primitive_init_info& info,
								u32 horizontal_index = axis::x,
								u32 vertical_index = axis::z,
								bool flip_winding = false,
								v3 offset = { -0.5f, 0.f, -0.5f },
								v2 u_range = { 0.f, 1.f },
								v2 v_range = { 0.f, 1.f })
		{
			...
			for (u32 i{ 0 }; i < num_indices; ++i) m.uv_sets[0].emplace_back(uvs[m.raw_indices[i]]);
		...
		void create_cube(scene& scene, const primitive_init_info&)
		{
		...
		void create_ico_sphere(scene& scene, const primitive_init_info&)
		{

		}

		void create_cylinder(scene& scene, const primitive_init_info&)
		{

		}

		void create_capsule(scene& scene, const primitive_init_info&)
		{
