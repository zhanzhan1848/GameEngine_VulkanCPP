...
/// <summary>
/// IF you gey any compilation or linker errors than make sure that
...
namespace primal::tools
{
	...
	bool fbx_context::get_mesh_data(FbxMesh* fbx_mesh, mesh & m)
	{
		...
		geometricTransform.SetT(node->GetGeometricTranslation(FbxNode::eDestinationPivot));
		geometricTransform.SetR(node->GetGeometricRotation(FbxNode::eDestinationPivot));
		geometricTransform.SetS(node->GetGeometricScaling(FbxNode::eDestinationPivot));
		...
		if (import_tangents)
		{
			...
			// Calculate tangents using FBX's built-in method, but only if no tangents data is already there.
			...
			{
				// something went wrong wtith importing tangents from FBX.
				// Fall back to our tangents calculation method
