...
namespace primal::utl
{
	...
	public:
		...
		// Constructor resize the vector and initializes 'count' items using 'value'
		...
		// Copy- or move-consrtucts an item at the end of the vector
		...
		// Same as erase() but faster beacuse it just copies the last item.
		...
		// Return a reference to the first item. Will fault the application if called
		// when the vector is empty
		[[nodiscard]] constexpr T& font()
		{
			assert(_data && _size);
			return _data[0];
		}

		// Return a constant reference to the first item. Will fault the application if called
		// when the vector is empty
		[[nodiscard]] constexpr const T& font() const
		{
			assert(_data && _size);
			return _data[0];
		}

		// Return a reference to the last item. Will fault the application if called
		// when the vector is empty
		...
		// Return a constant reference to the last item. Will fault the application if called
		// when the vector is empty
		...
		// Return a pointer to the first item. Returns null when vector is empty
		[[nodiscard]] constexpr T* begin()
		{
			return std::addressof(_data[0]);
		}

		// Return a constant pointer to the first item. Returns null when vector is empty
		[[nodiscard]] constexpr const T* begin() const
		{
			return std::addressof(_data[0]);
		}

		// Return a pointer to the last item. Returns null when vector is empty
		...
		// Return a constant pointer to the last item. Returns null when vector is empty
