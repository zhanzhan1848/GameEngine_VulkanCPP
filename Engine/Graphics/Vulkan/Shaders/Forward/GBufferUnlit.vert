...
// T4.6.5 part 12 — Vulkan port of Forward/GBufferUnlit.metal vertex stage.
// Entry point: main
//
// Vertex stage is identical to GBuffer.vert: the unlit difference is purely
// in the fragment stage. We keep a separate file so ForwardSceneRenderer can
// load "GBufferUnlit" by name and so future divergences (e.g. instance color
// passthrough) have a place to live.
//
...
// Full InstanceData struct (96B stride) matches C++ graphics::InstanceData layout;
// reading beyond transform (baseColor/roughness/metallic) when use_instances != 0.
struct InstanceData {
    mat4  transform;
    vec4  baseColor;
    float roughness;
