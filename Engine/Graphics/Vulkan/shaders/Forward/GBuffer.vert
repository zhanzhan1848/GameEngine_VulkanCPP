...
// T4.6.5 part 10 — Vulkan port of Forward/GBuffer.metal vertex stage.
// Entry point: main
//
...
//
// Note: ForwardSceneRenderer's GBuffer pipeline currently doesn't declare
// vertexBindings/vertexAttributes. Pipeline CREATION succeeds with empty
// vertex input state — runtime vertex data is undefined until C++ wires
// up VertexInputBinding/Attribute declarations. Tracked as follow-up.
...
void main() {
    ...
    // V-flip UV to match Metal convention (see memory: wgsl-y-negate-no-readback-flip
    // — Vulkan-derived shaders may need flip, but GBuffer.metal flips with 1.0 - rawUV.y)
