...
// Entry point: forwardWaterVS  →  main
//
...
//
// NOTE: GLSL std140 mandates vec3 align to 16B; C++ VertexInput uses packed_float3
// (12B) followed by VertexElement. The pipeline currently declares no vertex
// attributes (see GBuffer.vert comment) so runtime vertex data is undefined until
// C++ wires up VertexInputBinding/Attribute declarations. Tracked as follow-up.
...
// std140 layout trap — pack (use_instances + _pad[3]) into a single vec4 so the
// push constant block is exactly 64 + 16 = 80 bytes (matches PCGPushConsts).
...
void main() {
    ...
    // V-flip UV to match Metal convention.
