...
fn voxelize_sdf(@builtin(global_invocation_id) tid: vec3<u32>) {
    ...
    for (var inst_idx: u32 = 0u; inst_idx < num_instances; inst_idx++) {
        ...
        // VK override: use cascade-extent-proportional cull range instead of
        // the tight `voxel_size * 4.0` (which skips most geometry in large
        // scenes). cascade extent = resolution * voxel_size.
        let cascade_extent = f32(resolution.x) * voxel_size;
        ...
        if (dist_to_bounds > cascade_extent * 0.5) { continue; }
