...
void main() {
    ...
    // NO Y-negate. Although the GBuffer is rasterized by naga WGSL (which
    // auto-inserts a Y-negate), the HDR chain this quad composites into
    // follows the DeferredLighting fullscreen-triangle orientation (raw
    // GLSL). A/B measured: with the negate the floor quad lands at the TOP
    // of the frame; without it, on the floor where it belongs.
}
