		...
		private:
			...
			virtual void createPoolAndLayout()
			{
				...
				if ((_input_buffers.size() + _output_buffers.size()) > 0)
				{
					...
					poolSize.emplace_back(Engine_Descriptor_Pool_Size(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, frame_buffer_count * 3 * (storage_count + _output_buffers.size())));
					if(uniform_count != 0)
						poolSize.emplace_back(Engine_Descriptor_Pool_Size(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, static_cast<u32>(frame_buffer_count * 3 * (uniform_count))));
				}
				...
				if (_output_images.size() > 0)
					poolSize.emplace_back(Engine_Descriptor_Pool_Size(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, static_cast<u32>(frame_buffer_count * 3 * _output_buffers.size())));
		...
		private:
			...
			void createPoolAndLayout()
			{
				...
				if ((_input_buffers.size() + _output_buffers.size()) > 0)
				{
					...
					poolSize.emplace_back(Engine_Descriptor_Pool_Size(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, static_cast<u32>(frame_buffer_count * 3 * (storage_count + _output_buffers.size()))));
					if (uniform_count != 0)
						poolSize.emplace_back(Engine_Descriptor_Pool_Size(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, static_cast<u32>(frame_buffer_count * 3 * (uniform_count))));
				}
				if (_input_images.size() > 0)
