...
struct SceneData {
    ...
    float  time;        // Wind animation time
    float  _timePad;
    float4 viewPos;
...
vertex VertexOut vertexMain(
    ...
    // Wind animation: sinusoidal displacement on XZ based on height
    float heightFactor = saturate(rawPos.y / 2.0);
    float windX = sin(sceneData.time * 1.5 + worldPos.x * 0.8 + worldPos.z * 0.3) * 0.15 * heightFactor;
    float windZ = sin(sceneData.time * 1.2 + worldPos.z * 0.6 + worldPos.x * 0.4) * 0.15 * heightFactor;
    worldPos.x += windX;
    worldPos.z += windZ;
...
fragment FragmentOut fragmentMain(
    ...
    // Alpha discard for leaf textures
