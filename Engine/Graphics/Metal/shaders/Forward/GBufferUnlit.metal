...
fragment FragmentOut fragmentMain(
    ...
    // Unlit: output base color directly, no lighting
    out.albedo = in.instanceBaseColor;

    // Flat normal pointing +Z (encode as 0.5, 0.5, 1.0)
    out.normal = float4(0.5, 0.5, 1.0, 1.0);

    // ORM: AO=1, Roughness=1, Metallic=0
    out.orm = float4(1.0, 1.0, 0.0, 1.0);
