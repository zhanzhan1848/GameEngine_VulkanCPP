...
// Plane
float SDPlane( float3 position )
{
	return position.y;
}

// Sphere
float SDSphere( float3 position, float center )
{
    return length(position) - center;
}

// Box
float SDBox( float3 position, float3 center )
{
    float3 d = abs(position) - center;
    return min(max(d.x, max(d.y, d.z)), 0.0) + length(max(d, 0.0));
}

// Box Frame
float SDBoxFrame( float3 position, float3 center, float e )
{
    position = abs(position) - center;
    float3 q = abs(position + e) - e;

    return min(min(
        length(max(float3(position.x, q.y, q.z), 0.0))+min(max(position.x, max(q.y, q.z)), 0.0),
        length(max(float3(q.x, position.y, q.z), 0.0))+min(max(q.x, max(position.y, q.z)), 0.0)),
        length(max(float3(q.x, q.y, position.z), 0.0))+min(max(q.x, max(q.y, position.z)), 0.0));
}
...
float SDTriPrism( float3 p, float2 h )
{
    ...
    return length(max(float2(d1, d2), 0.0f)) + min(max(d1, d2), 0.f);
}
