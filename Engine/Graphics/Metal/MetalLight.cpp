...
#define CONSTEXPR constexpr
        ...
        {
        public:
            graphics::light add(light_init_info& info)
            {
                if(info.type == graphics::light::directional)
                {
                    ...
                    msl::DirectionalLightParameters& params{ _non_cullable_lights[index] };
					params.Color = math::v4{ info.color.x, info.color.y, info.color.z, 0.f };
					params.DirectionAndIntensity.w = info.intensity;
            ...
            void update_transforms()
            {
				...
				for (const auto& id : _non_cullable_owners)
				{
					...
					if (owner.is_enabled)
					{
						...
						msl::DirectionalLightParameters& params{ _non_cullable_lights[owner.data_index] };
						params.DirectionAndIntensity.xyz = entity.orientation();
#if SHADOW_MAPPING
						// Metal使用右手坐标系，需要调整视图和投影矩阵
						math::m4x4 model = matrix_identity_float4x4;
						
						// 使用右手坐标系的LookAt矩阵，光源位置向后偏移
						math::v3 lightPos = math::v3{ 0.0f, 0.0f, 0.0f } - params.DirectionAndIntensity.xyz * 32.0f;
						math::m4x4 view = math::createLookToLH(lightPos, params.DirectionAndIntensity.xyz, math::v3{ 0.0f, 1.0f, 0.0f });
						
						// Metal的正交投影矩阵，调整近远平面范围
						math::m4x4 projection = math::createOrthographicLH(32.f, 32.f, 0.1f, 1000.0f);
						
						params.LightMVP = simd_mul(projection, simd_mul(view, model));
#endif
					}
				}
            ...
            constexpr void intensity(light_id id, f32 intensity)
			{
				...
				if (owner.type == graphics::light::directional)
				{
					...
					_non_cullable_lights[index].DirectionAndIntensity.w = intensity;
				}
				else
				{
            ...
            void color(light_id id, math::v3 color)
			{
				...
				{
					assert(index < _non_cullable_lights.size());
					_non_cullable_lights[index].Color = math::v4{ color.x, color.y, color.z, 0.f };
				}
				else
				{
					assert(_owners[_cullable_owners[index]].data_index == index);
            ...
            void attenuation(light_id id, math::v3 attenuation)
			{
            ...
            void umbra(light_id id, f32 umbra)
			{
				...
				_cullable_lights[index].CosUmbra = std::cos(umbra * 0.5f);
				make_dirty(index);
			...
			void penumbra(light_id id, f32 penumbra)
			{
				...
				_cullable_lights[index].CosPenumbra = std::cos(penumbra * 0.5f);
				calculate_cone_bounding_sphere(_cullable_lights[index], _bounding_spheres[index]);
			...
			constexpr f32 intensity(light_id id) const
			{
				...
				{
					assert(index < _non_cullable_lights.size());
					return _non_cullable_lights[index].DirectionAndIntensity.w;
				}
			...
			math::v3 color(light_id id) const
			{
				const light_owner& owner{ _owners[id] };
				const u32 index{ owner.data_index };
				...
				{
					assert(index < _non_cullable_lights.size());
					return _non_cullable_lights[index].Color.xyz;
				}
			...
			math::v3 attenuation(light_id id) const
			{
				const light_owner& owner{ _owners[id] };
				const u32 index{ owner.data_index };
				assert(_owners[_cullable_owners[index]].data_index == index);
			...
			f32 umbra(light_id id) const
			{
				...
				assert(owner.type == graphics::light::spot);
				assert(index < _cullable_lights.size());
				return std::acos(_cullable_lights[index].CosUmbra) * 2.f;
			}
			...
			f32 penumbra(light_id id) const
			{
				...
				assert(owner.type == graphics::light::spot);
				assert(index < _cullable_lights.size());
				return std::acos(_cullable_lights[index].CosPenumbra) * 2.f;
			}
            ...
            CONSTEXPR void non_cullable_lights(msl::DirectionalLightParameters *const lights, [[maybe_unused]] u32 buffer_size) const
			{
				assert(buffer_size >= math::align_size_up<256>(non_cullable_light_count() * sizeof(msl::DirectionalLightParameters)));
        ...
        private:
            ...
            void calculate_cone_bounding_sphere(const msl::LightParameters& params, msl::Sphere& sphere)
			{
				math::v3 tip{ params.Position };
				math::v3 direction{ params.Direction };
				...
				{
                    sphere.Center = tip + cone_cos * params.Range * direction;
            ...
            void update_transform(u32 index)
			{
				...
				msl::LightParameters& params{ _cullable_lights[index] };
				params.Position = entity.position();

				msl::LightCullingLightInfo& culling_info{ _culling_info[index] };
            ...
            CONSTEXPR void add_cullable_light_parameters(const light_init_info& info, u32 index)
			{
				...
				else if (info.type == light::spot)
				{
					...
					params.Attenuation = p.attenuation;
					params.Range = p.range;
					params.CosUmbra = std::cos(p.umbra * 0.5f);
					params.CosPenumbra = std::cos(p.penumbra * 0.5f);
				}
			}
            ...
            CONSTEXPR void add_light_culling_info(const light_init_info& info, u32 index)
			{
				...
				const msl::LightParameters& params{ _cullable_lights[index] };
			...
			utl::vector<msl::DirectionalLightParameters>		_non_cullable_lights;
			utl::vector<light_id>								_non_cullable_owners;

			// NOTE: these are tightly packed
			utl::vector<msl::LightParameters>					_cullable_lights;
			utl::vector<msl::LightCullingLightInfo>			    _culling_info;
			utl::vector<msl::Sphere>							_bounding_spheres;
			...
			u8													_something_is_dirty{ 0 };  // flag is set if any of cullable lights were changed.

			friend class metal_light_buffer;
        };

        class metal_light_buffer
        {
        public:
            metal_light_buffer() = default;
            CONSTEXPR void update_light_buffers(light_set& set, u64 light_set_key, u32 frame_index)
			{
				...
				{
					const u32 needed_size{ non_cullable_light_count * static_cast<u32>(sizeof(msl::DirectionalLightParameters)) };
					...
					set.non_cullable_lights((msl::DirectionalLightParameters *const)_buffers[light_buffer::non_cullable_light].cpu_address,
						_buffers[light_buffer::non_cullable_light].buffer.size());
					// _buffers[light_buffer::non_cullable_light].buffer.buffer()->didModifyRange(NS::Range::Make(0, _buffers[light_buffer::non_cullable_light].buffer.size()));
				}
				...
				{
					const u32 needed_light_buffer_sizes{ cullable_light_count * static_cast<u32>(sizeof(msl::LightParameters)) };
					const u32 needed_culling_buffer_sizes{ cullable_light_count * static_cast<u32>(sizeof(msl::LightCullingLightInfo)) };
					const u32 needed_spheres_buffer_size{ cullable_light_count * static_cast<u32>(sizeof(msl::Sphere)) };
					...
					if (current_light_buffer_size < needed_light_buffer_sizes)
					{
						...
						resize_buffer(light_buffer::cullable_light, (needed_light_buffer_sizes * 3) >> 1, frame_index);
						resize_buffer(light_buffer::culling_info, (needed_culling_buffer_sizes * 3) >> 1, frame_index);
						resize_buffer(light_buffer::bounding_spheres, (needed_spheres_buffer_size * 3) >> 1, frame_index);
						buffers_resized = true;
					}

					const u32 index_mask{ static_cast<u32>(1UL << frame_index) };
					if (buffers_resized || _current_light_set_key != light_set_key)
					{
						memcpy(_buffers[light_buffer::cullable_light].cpu_address, set._cullable_lights.data(), needed_light_buffer_sizes);
						memcpy(_buffers[light_buffer::culling_info].cpu_address, set._culling_info.data(), needed_culling_buffer_sizes);
					...
					else if(set._something_is_dirty)
					{
						for (u32 i{ 0 }; i < cullable_light_count; ++i)
						{
							...
							{
								assert(i * sizeof(msl::LightParameters) < needed_light_buffer_sizes);
								assert(i * sizeof(msl::LightCullingLightInfo) < needed_culling_buffer_sizes);
								u8* const light_dst{ _buffers[light_buffer::cullable_light].cpu_address + (i * sizeof(msl::LightParameters)) };
								u8* const culling_dst{ _buffers[light_buffer::culling_info].cpu_address + (i * sizeof(msl::LightCullingLightInfo)) };
								u8* const bounding_dst{ _buffers[light_buffer::bounding_spheres].cpu_address + (i * sizeof(msl::Sphere)) };
								memcpy(light_dst, &set._cullable_lights[i], sizeof(msl::LightParameters));
								memcpy(culling_dst, &set._culling_info[i], sizeof(msl::LightCullingLightInfo));
								memcpy(bounding_dst, &set._bounding_spheres[i], sizeof(msl::Sphere));
								set._dirty_bits[i] &= ~index_mask;
							}
						}
					}
            ...
            constexpr MTL::Buffer* non_cullable_lights() const { return _buffers[light_buffer::non_cullable_light].buffer.buffer(); }
			constexpr MTL::Buffer* cullable_lights() const { return _buffers[light_buffer::cullable_light].buffer.buffer(); }
			constexpr MTL::Buffer* culling_info() const { return _buffers[light_buffer::culling_info].buffer.buffer(); }
			constexpr MTL::Buffer* bounding_spheres() const { return _buffers[light_buffer::bounding_spheres].buffer.buffer(); }
        private:
            struct light_buffer
			{
				enum type : u32
            ...
            void resize_buffer(light_buffer::type type, u32 size, [[maybe_unused]] u32 frame_index)
			{
				...
				if (!size || _buffers[type].buffer.size() >= math::align_size_up<256>(size)) return;

				new (&_buffers[type].buffer) metal_buffer{ constant_buffer::get_default_init_info(size), true };
				NAME_METAL_OBJECT_INDEXED(_buffers[type].buffer.buffer(), frame_index,
					type == light_buffer::non_cullable_light ? "Non-cullable Light Buffer" :
					type == light_buffer::cullable_light ? "Cullable Light Buffer" : 
					type == light_buffer::culling_info ? "Culling Info Buffer" : "Bounding Spheres Buffer");

				_buffers[type].buffer.buffer()->setLabel(NS::String::string(
					type == light_buffer::non_cullable_light ? "Non-cullable Light Buffer" :
					type == light_buffer::cullable_light ? "Cullable Light Buffer" : 
					type == light_buffer::culling_info ? "Culling Info Buffer" : "Bounding Spheres Buffer", NS::UTF8StringEncoding));
				_buffers[type].cpu_address = (u8*)_buffers[type].buffer.buffer()->contents();
		...
		metal_light_buffer							light_buffers[frame_buffer_count];
		...
		void set_color(light_set& set, light_id id, const void *const data, [[maybe_unused]] u32 size)
		{
		...
		void set_attenuation(light_set& set, light_id id, const void *const data, [[maybe_unused]] u32 size)
		{
		...
		void get_color(const light_set& set, light_id id, void *const data, [[maybe_unused]] u32 size)
		{
		...
		void get_attenuation(const light_set& set, light_id id, void *const data, [[maybe_unused]] u32 size)
		{
...
#undef CONSTEXPR
	...
	{
		const u64 light_set_key{ metal_info.info->light_set_key };
		assert(light_sets.count(light_set_key));
		...
		metal_light_buffer& light_buffer{ light_buffers[frame_index] };
		light_buffer.update_light_buffers(set, light_set_key, frame_index);
	}

    MTL::Buffer* non_cullable_light_buffer(u32 frame_index)
	{
		const metal_light_buffer& light_buffer{ light_buffers[frame_index] };
