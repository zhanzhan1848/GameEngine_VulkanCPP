...
namespace primal::graphics::d3d12::d3dx
{
	...
	ID3D12Resource * create_buffer(const void* data, u32 buffer_size, bool is_cpu_accessible, D3D12_RESOURCE_STATES state, D3D12_RESOURCE_FLAGS flags, ID3D12Heap * heap, u64 heap_offset)
	{
		...
		if (data)
		{
			...
			// to true. If we only want to upload scene data to be used by the GPU, then is_cpu_accessible
			// should be set to false
			if (is_cpu_accessible)
			{
				// NOTE: range's Begin and End fields are set to 0, to indicate that 
				//		 the CPU is not reading and data (i.e. write-only)
			...
			{
				// We need a upload context here. This is for uploading resources
				// that are only accessed by the GPU
