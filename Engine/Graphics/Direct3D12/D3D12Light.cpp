...
#define CONSTEXPR constexpr
		...
		public:
			...
			CONSTEXPR void update_light_buffers(light_set& set, u64 light_set_key, u32 frame_index)
			{
				...
				{
					const u32 needed_light_buffer_sizes{ cullable_light_count * sizeof(hlsl::LightParameters) };
					const u32 needed_culling_buffer_sizes{ cullable_light_count * sizeof(hlsl::LightCullingLightInfo) };
					...
					else if(set._something_is_dirty)
					{
						for (u32 i{ 0 }; i < cullable_light_count; ++i)
						{
							...
							{
								assert(i * sizeof(hlsl::LightParameters) < needed_light_buffer_sizes);
								assert(i * sizeof(hlsl::LightCullingLightInfo) < needed_culling_buffer_sizes);
		...
		private:
			...
			void resize_buffer(light_buffer::type type, u32 size, [[maybe_unused]] u32 frame_index)
			{
				...
				NAME_D3D12_OBJECT_INDEXED(_buffers[type].buffer.buffer(), frame_index,
					...
					type == light_buffer::culling_info ? L"Culling Info Buffer" : L"Bounding Spheres Buffer");
