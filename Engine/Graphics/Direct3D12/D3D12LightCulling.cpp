...
namespace primal::graphics::d3d12::delight
{
	namespace
	{
		...
		struct culling_parameters
		{
			...
			hlsl::LightCullingDispatchParameters		light_culling_diapatch_params{};
			u32											frustum_count{ 0 };
			u32											view_height{ 0 };
			u32											view_width{ 0 };
			f32											camera_fov{ 0.f };
		...
		bool create_root_signature()
		{
			...
			// 对应gridFrustums shader中的 register(b0，space0)
			// 其中使用as_cbv向shader传入参数
			...
			// 使用as_uav从shader中输出， 对应register(b0, space0)
		...
		{
			constexpr u32 tile_size{ light_culling_tile_szie };
			...
			{
				hlsl::LightCullingDispatchParameters& params{ culler.light_culling_diapatch_params };
	...
	bool initialize()
	{
		return create_root_signature() && create_pso() && light::initialize();
	}
