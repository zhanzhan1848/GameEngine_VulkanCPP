...
namespace primal::graphics::d3d12::core
{
	namespace
	{
		...
		public:
			...
			void end_frame(const d3d12_surface& surface)
			{
				...
				// Presentimng swap chain buffers happens in lockstep with frame buffers.
				surface.present();
		...
		private:
			struct command_frame
			{
				...
				void wait(HANDLE fence_event, ID3D12Fence* fence)
				{
					...
					{
						// We have the fence create an event which is a singaled one the fence's current value equals "fence_value"
						DXCall(fence->SetEventOnCompletion(fence_value, fence_event));
						// Wait untill the fence has triggered the event that its current value has reached "fence_value"
		...
		void __declspec(noinline) process_deferred_release(u32 frame_idx)
		{
	...
	bool initialize()
	{
		// determine what is the maxmium feature level that is supporter
...
#if 0
#pragme message("WARNING: GPU based validation is enabled. This will considerably slow down the renderer!")
					debug_interface->SetEnableGPUBasedValidation(1);
#endif
			}
			else
			{
				OutputDebugStringA("Warning: D3D12 Debug interface is not available. Verify that Graphics Tools optional feature is installed int this device");
			}
...
#ifdef _DEBUG
		{
			...
			// TODO: Remove these( to aviod some question )
	...
	void shutdown()
	{
		...
		// NOTR: some modules free theri descriptors when they shutdopwn
