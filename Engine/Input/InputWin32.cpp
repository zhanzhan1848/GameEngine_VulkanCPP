...
namespace primal::input
{
	namespace
	{
		constexpr u32 vk_mapping[256]
		{
			...
			/* 0xDD */ input_code::key_brack_close,
		...
		void set_modifier_inputs(input_code::code code)
		{
			...
			{
				set_modifier_input(VK_LMENU, input_code::key_left_atl, modifier_flags::left_atl);
				set_modifier_input(VK_RMENU, input_code::key_right_atl, modifier_flags::right_atl);
			}
		}
	...
	HRESULT process_input_message(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
	{
		...
		{
			SetCapture(hwnd);
			const input_code::code code{ msg == WM_LBUTTONDOWN ? input_code::mouse_left : msg == WM_RBUTTONDOWN ? input_code::mouse_rigth : input_code::mouse_middle };
			const math::v2 pos{ get_mouse_position(lparam) };
		...
		{
			ReleaseCapture();
			const input_code::code code{ msg == WM_LBUTTONUP ? input_code::mouse_left : msg == WM_RBUTTONUP ? input_code::mouse_rigth : input_code::mouse_middle };
			const math::v2 pos{ get_mouse_position(lparam) };
