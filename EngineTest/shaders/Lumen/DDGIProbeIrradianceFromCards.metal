...
#include "Lumen/DDGIVolumeData.metal"
...
kernel void ddgi_probe_irradiance_from_cards(
    ...
    const device auto& range = ranges[probeIdx];
    ...
    for (uint i = 0; i < range.count && i < 6u; ++i) {
        const device auto& c = contribs[range.start + i];
    ...
    // Sky ambient baseline: ensures non-zero irradiance even when card radiance
    // is near-zero in shadow areas. Approximates the miss-ray contribution that
    // the old TraceRays+UpdateIrradiance pipeline provided via DDGI_SKY_COLOR.
    sh[0] += DDGI_SKY_COLOR * 0.05f;
