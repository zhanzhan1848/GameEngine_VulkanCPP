...
vertex ShadowDepthVertexOut shadow_depth_vs(
    ...
    // Reconstruct float4x4 manually to guarantee 100% correct packing
    float4x4 lightVP = float4x4(
        float4(uniforms.light_view_projection[0], uniforms.light_view_projection[1], uniforms.light_view_projection[2], uniforms.light_view_projection[3]),
        float4(uniforms.light_view_projection[4], uniforms.light_view_projection[5], uniforms.light_view_projection[6], uniforms.light_view_projection[7]),
        float4(uniforms.light_view_projection[8], uniforms.light_view_projection[9], uniforms.light_view_projection[10], uniforms.light_view_projection[11]),
        float4(uniforms.light_view_projection[12], uniforms.light_view_projection[13], uniforms.light_view_projection[14], uniforms.light_view_projection[15])
    );

    // Transform: local → world → light clip space
    float4 world_pos = inst.world_matrix * float4(local_pos, 1.0f);
    out.position = lightVP * world_pos;
