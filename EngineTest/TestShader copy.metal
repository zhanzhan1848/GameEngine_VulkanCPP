                             ...
                             uint vertex_index [[vertex_id]])
{
    ...
    float3 normal = float3(nXY.x, nXY.y, sqrt(clamp(1.f - dot(nXY, nXY), 0.f, 1.f)) * nSign);
...
{
    float4 color;
    float3 normalizedPos = normalize(vsOut.WorldPosition);
    color = float4(normalizedPos * 0.5f + 0.5f, 1.f);
