...
float3 PointLight(float3 N, float3 worldPosition, float3 V, LightParameters light)
{
	...
	if (dSq < light.Range * light.Range)
	{
		...
		color = CalculateLighting(N, L, V, attenuation * light.Color * light.Intensity * 0.2f);
	}
#endif
    return color;
}
...
float3 SpotLight(float3 N, float3 worldPosition, float3 V, LightParameters light)
{
	...
	if (dSq < light.Range * light.Range)
	{
		...
		color = CalculateLighting(N, L, V, light.Color * light.Intensity * attenuation * angularAttenuation * 0.02f);
	}
#endif
    return color;
}
