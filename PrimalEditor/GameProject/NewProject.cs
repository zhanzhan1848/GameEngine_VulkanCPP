...
namespace PrimalEditor.GameProject
{
    ...
    class NewProject : ViewModelBase
    {
        ...
        private bool ValidateProjectPath()
        {
            ...
            {
                ErrorMsg = "Inavlid character(s) used in project name."; 
            }
            else if (string.IsNullOrWhiteSpace(ProjectPath.Trim()))
            {
                ErrorMsg = "Type in a project folder.";
            }
            else if (ProjectPath.IndexOfAny(Path.GetInvalidPathChars()) != -1)
            {
                ErrorMsg = "Inavlid character(s) used in project path.";
            }
        ...
        {
            System.Diagnostics.Debug.Assert(File.Exists(Path.Combine(template.TemplatePath, "MSVCSolution")));
            System.Diagnostics.Debug.Assert(File.Exists(Path.Combine(template.TemplatePath, "MSVCProject")));

            var engineAPIPath = Path.Combine(MainWindow.PrimalPath, @"Engine\EngineAPI\");
            System.Diagnostics.Debug.Assert(Directory.Exists(engineAPIPath));
            ...
            solution = string.Format(solution, _0, _1, _1);
        ...
        public NewProject()
        {
            ...
            try
            {
                ...
                System.Diagnostics.Debug.Assert(templatesFiles.Any());
                foreach (var file in templatesFiles)
                {
                    var template = Serializer.FromFile<ProjectTemplate>(file);
                    template.TemplatePath = Path.GetDirectoryName(file);
                    template.IconFilePath = System.IO.Path.GetFullPath(Path.Combine(template.TemplatePath, "Icon.png"));
                    template.Icon = File.ReadAllBytes(template.IconFilePath);
                    template.ScreenshotFilePath = System.IO.Path.GetFullPath(Path.Combine(template.TemplatePath, "Screenshot.png"));
